...
import type { CreateFeedPostParams, FeedPost, FeedPostMediaItem } from '../types/feed-post';
...
interface FeedPostComposerCommonProps extends Pick<FastCommentsCallbacks, 'pickImage' | 'pickGIF' | 'onError'> {
 ...
 */
export type FeedPostComposerProps = FeedPostComposerCommonProps &
 ...
 */
export function FeedPostComposer(props: FeedPostComposerProps) {
    ...
    const canSubmit = !busy && (hasText || attached.length > 0 || title.trim().length > 0);
    ...
    return (
        <View style={f?.composer} testID="feedPostCreate" accessibilityLabel="feedPostCreate">
            ...
            <CommentTextArea
                ...
                pickImage={pickImage}
                pickGIF={pickGIF}
                // Image/GIF live in the WYSIWYG toolbar like the comment editor, but
                // onAttachMedia routes the picked media to the post (separate
                // attachment list, Android-style) instead of into the post text.
                toolbarButtons={{ bold: true, italic: true, underline: true, strikethrough: true, code: false, image: true, gif: true }}
                onAttachMedia={addMedia}
            />
